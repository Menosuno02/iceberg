...
def room_enroll(request):
    ...
    all_rooms = Room.objects.filter(Q(author=request.user) | Q(
        friend=request.user)).order_by('-created')
    context = {
        'all_rooms': all_rooms,
        'all_friends': friends,
    }
...
def room_choice(request, friend_id):
    ...
    if not FriendList.objects.filter(user=request.user, friends=friend[0]):
        ...
        return redirect('room-enroll')
    room = Room.objects.filter(Q(author=request.user, friend=friend[0]) | Q(
